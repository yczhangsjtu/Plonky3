...
use p3_util::{log2_strict_usize, reverse_index_bits_in_place};
...
pub fn fft_root_table<F: Field + TwoAdicField>(n: usize) -> FftRootTable<F> {
...
#[inline]
fn fft_dispatch<F: Field + TwoAdicField>(
    input: &mut [F],
...
fn fft_classic_simd<P: PackedField>(
    ...
    let lg_packed_width = log2_strict_usize(P::WIDTH); // 0 when P is a scalar.
...
pub(crate) fn fft_classic<F: Field>(values: &mut [F], r: usize, root_table: &FftRootTable<F>) {
    ...
    let lg_packed_width = log2_strict_usize(<F as Packable>::Packing::WIDTH);
